...
$getcontent.Replace("http://localhost:8080","$result").Replace("testkey","$StorageAccountKey").Replace("passwordjenkins","$passwordjenkins").Replace("subscriptionid","$subscriptionID").Replace("clientid","$ApplicationId").Replace("appSecret","$ClientSecret").Replace("tenantid","$TenantId").Replace("testuser","$ContainerAdminUser").Replace("testpassword","$ContainerAdminPassword").Replace("cd /home/vname","cd /home/$username") | Out-File 'C:/Users/admin/Desktop/credentials_update.sh'
